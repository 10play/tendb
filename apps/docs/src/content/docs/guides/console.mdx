...
import branchesDark from '../../../assets/console/branches-dark.png';
import branchesLight from '../../../assets/console/branches-light.png';
import snapshotsDark from '../../../assets/console/snapshots-dark.png';
import snapshotsLight from '../../../assets/console/snapshots-light.png';
import alertsDark from '../../../assets/console/alerts-dark.png';
import alertsLight from '../../../assets/console/alerts-light.png';
import tablesDark from '../../../assets/console/tables-dark.png';
import tablesLight from '../../../assets/console/tables-light.png';
import sqlDark from '../../../assets/console/sql-dark.png';
import sqlLight from '../../../assets/console/sql-light.png';
import monitoringDark from '../../../assets/console/monitoring-dark.png';
import monitoringLight from '../../../assets/console/monitoring-light.png';
...
<ConsoleShot dark={dashDark} light={dashLight} alt="The console dashboard" caption="The dashboard: getting-started cards, branch/storage/sync tiles, the branch list, and platform settings." />
...
<ConsoleShot dark={branchesDark} light={branchesLight} alt="The branches table" caption="Branches: lineage, state, ports, data state, and per-row connect / reset / delete actions." />
...
**Snapshots** — cards for sync status, disk (with warning tones above 80% and danger above 92%), data state, and upstream replication (stream health, slot state, lag bytes, WAL retained per slot, apply/sync error counters). On streaming deployments you also get a **Snapshot now** button (an O(1) ZFS snapshot — around ten seconds regardless of database size) and a **snapshot schedule** card — interval choices from manual to daily, retention 1–500 — saved to SSM and picked up by the engine host within seconds. On dump-mode deployments a **Refresh now** button triggers a full dump/restore refresh when the engine supports it. Below the cards, the snapshot table lists id, data state, age, and logical/physical sizes. See [Data refresh](/concepts/data-refresh/) for how the two sync modes differ.

<ConsoleShot dark={snapshotsDark} light={snapshotsLight} alt="The snapshots screen" caption="Snapshots: sync, disk, and data-state cards over the snapshot table with per-branch ZFS sizes." />

**Alerts** — current health findings with severity pills, plus:

- **Schema card** (streaming): drift status and a **Force schema sync** button for a full reconcile — destructive (it drops tables removed upstream), so it asks you to type `approve` first. The auto-heal daemon toggle lives in the CLI: `tendb schema config --auto-sync on|off`.
...
<ConsoleShot dark={alertsDark} light={alertsLight} alt="The alerts screen" caption="Alerts: current findings, the schema card, Slack webhook setup, and the event feed." />
...
<ConsoleShot dark={tablesDark} light={tablesLight} alt="The schema browser" caption="Tables: size bars, column details, and a 50-row preview — scoped to the working branch." />
...
<ConsoleShot dark={sqlDark} light={sqlLight} alt="The SQL editor" caption="The SQL editor running an aggregate on a branch: result grid, timing, and the history sidebar." />
...
<ConsoleShot dark={monitoringDark} light={monitoringLight} alt="The monitoring screen" caption="Monitoring: database size, cache hit ratio, connections, and top queries for one branch." />
